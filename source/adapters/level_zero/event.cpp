...
void printZeEventList(const _ur_ze_event_list_t &UrZeEventList) {
  if (UrL0Debug & UR_L0_DEBUG_BASIC) {
    logger::debug("  NumEventsInWaitList %d:", UrZeEventList.Length);

    for (uint32_t I = 0; I < UrZeEventList.Length; I++) {
      logger::debug(" %#llx", ur_cast<std::uintptr_t>(UrZeEventList.ZeEventList[I]));
    }
...
UR_APIEXPORT ur_result_t UR_APICALL urEventGetInfo(
  ...
  default:
    logger::debug("Unsupported ParamName in urEventGetInfo: ParamName=%d(%x)\n",
            PropName, PropName);
...
UR_APIEXPORT ur_result_t UR_APICALL urEventGetProfilingInfo(
  ...
  default:
    logger::debug("urEventGetProfilingInfo: not supported ParamName\n");
    return UR_RESULT_ERROR_INVALID_VALUE;
  }
